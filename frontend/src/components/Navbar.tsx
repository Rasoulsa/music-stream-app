...
export default function Navbar() {
  ...
  const logoutMessageTimer = useRef<ReturnType<typeof setTimeout> | null>(null);

  useEffect(() => {
    return () => {
      if (logoutMessageTimer.current) {
        clearTimeout(logoutMessageTimer.current);
      }
    };
  }, []);
  ...
  const handleLogout = () => {
    ...
    logoutMessageTimer.current = setTimeout(() => {
      setShowLogoutMessage(false);
      logoutMessageTimer.current = null;
    }, 4_000);
  };
    ...
    <>
      ...
      {showLogoutMessage && (
        <div
          role="status"
          aria-live="polite"
          className="fixed top-20 right-6 z-[60] rounded-lg border border-green-500/30 bg-[var(--surface-2)] px-5 py-3 text-sm text-green-400 shadow-lg"
        >
          You have signed out.
        </div>
      )}
